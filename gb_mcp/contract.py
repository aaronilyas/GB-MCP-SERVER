...
HOW_TO_PLAY_MAX_CHARS = 2200

HOW_TO_PLAY = """\
Game Boy MCP.
Loop: list_games → boot → play → look at the LAST returned image → repeat.
...
Walk: omit frames on a single D-pad to hold-walk (default 1800); look at the LAST image only. Pass frames=16 to tap one tile or line up.
Holds abort on combat HUD, text, menu, fade, or blocked. After boot logos use intent=skip_intro.
Dialogue: intent=advance_text until the box stays gone. Never hold A.
If looks_like=battle, use intent=battle_turn or intent=battle_until_overworld. Do not open START on a fight HUD.
When facing a door, use intent=enter_door.
If stopped_reason=blocked, change direction. A short tap may only turn facing; that is not blocked.
Do not request GIFs or scale-4 shots. The server logs inputs; do not write frame-by-frame diaries.
