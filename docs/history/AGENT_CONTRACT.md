...
Frozen defaults, public request/response, Vision classifiers, and Default
hold abort below match the 2026-09-05 play-session LCD rewrite. The
multi-agent file-ownership / sub-agent A–F split is historical (2026-09-04)
and is not a second hold-abort definition.
...
## Historical multi-agent split (2026-09-04)

The file-ownership table, sub-agent A–F work split, and input-engine API
sketch below were the coordination record for landing `571331c`. They are not
a living assignment. `gb_mcp/emulator/vision.py` already existed then; it is
not a new file. This contract lives at `docs/history/AGENT_CONTRACT.md`
(there is no `docs/AGENT_CONTRACT.md`). 2026-09-05 play-session LCD /
two-gate abort / diagnostic `window_occluded_likely` rules are in Frozen
defaults, Vision rules, and Default hold abort above, plus README.md and
CHANGELOG.md — do not treat this ownership table as a second hold-abort
definition.

| Owner | May write | Must not write |
| --- | --- | --- |
| **Lead** | `docs/history/AGENT_CONTRACT.md`, `gb_mcp/emulator/play_limits.py`, `gb_mcp/emulator/input_schema.py`, final merge of overlapping files, `IMPLEMENTATION_REPORT.md`, default-hold-abort decision | — |
| **A — input + speed** | `gb_mcp/emulator/input_engine.py`, `tests/test_input_engine.py` | `vision.py`, `server.py`, MCP schema text, idle timeout numbers, classifiers |
| **B — framebuffer** | `gb_mcp/emulator/vision.py`, `tests/test_vision.py`, optional `tests/fixtures/lcd/*.png` | button scheduling, session timeout, `server.py` |
| **C — session** | `gb_mcp/config.py` (idle default), `gb_mcp/emulator/loop.py` (**ping/save/idle-wait/speed-on-start only** — do not rewrite `_apply_input`), `gb_mcp/emulator/session.py`, `gb_mcp/emulator/backend.py`, `gb_mcp/emulator/instance.py`, `docker/instance_server.py`, `tests/test_session_lifecycle.py` | `vision.py`, `input_engine.py`, framebuffer math |
...
| **F — OCR** | `gb_mcp/emulator/ocr.py`, optional extra in `requirements-instance.txt` | default path of input |
